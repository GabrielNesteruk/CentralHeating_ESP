...
void ConfigurationManager::Update(const ReportStationConfigData& data)
{
	memcpy(&this->config_data.data, &data, sizeof(ReportStationConfigData));
	this->config_data.crc =
		crc16_CCITT(reinterpret_cast<uint8_t*>(&this->config_data), sizeof(this->config_data) - 2);
	this->Save();
}

ReportStationConfigData ConfigurationManager::Get() const
{
